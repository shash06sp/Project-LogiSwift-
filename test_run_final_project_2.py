...
def solve_complete_project_with_viz():
    ...
    DEPOT_LATLON = [12.93580, 77.62590]
    ...
    for cid in range(NUM_CUSTOMERS):
        ...
        customer_points.append({
            ...
            "Latitude": DEPOT_LATLON[0] + dx * np.cos(angle),
            "Longitude": DEPOT_LATLON[1] + dx * np.sin(angle)
        })

    ordersTable = pd.DataFrame(customer_points)

    # Combine depot + orders into one big list
    all_nodes = [DEPOT_LATLON] + list(zip(ordersTable["Latitude"], ordersTable["Longitude"]))
    ...
    if sol:
        ...
        create_solution_map(routes, usable_orders, DEPOT_LATLON)
    else:
        print("\n!!! No solution found :( !!!")
